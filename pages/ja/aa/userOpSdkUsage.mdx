...
以下では、userop-sdkの使い方や、DApp側でどのように userOpを組み立てて Paymaster-RPCによるガス支払いロジックを取得し、最終的にバンドラーによってオンチェーン化するまでの流れを解説します。
...
const builder = await Presets.Builder.SimpleAccount.init(
    ...
    {
      overrideBundlerRpc: bundlerRpc,    // バンドラーRPC
...
```
設定を変更した後、再度トランザクションデータを組み立て、Paymaster の署名要求を行います。そして最終的にバンドラーに送信し、オンチェーンにアップロードします。

## UserOperation をバンドラーへ送信

```typescript
const res = await client.sendUserOperation(builder)
```
sendUserOperation を呼び出すと、内部で Paymaster への署名リクエストを行い、UserOp を完成させたうえでバンドラーに提出します。バンドラーによってオンチェーン化されると、ガス支払いロジックが適用されたトランザクションが実行されます。
