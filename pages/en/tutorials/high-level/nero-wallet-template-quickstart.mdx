...
# Nero Wallet Building Tutorial: Adding NFT & Token Features to NERO Wallet Template

This tutorial will guide you through the process of enhancing the NERO Wallet template with NFT minting, token creation, and an NFT gallery.
...
```
2. Have an AA-platform API Key. See how to here:

### Optional
You can enable social logins in this example. Go to the web3auth website and generate a clientId with your desired social provider (Google, Facebook, etc.). 
...
1.  Create a new file src/HomePage.tsx with three tabs: NFT Minting, Token Creation, and NFT Gallery.

2.  Copy this basic structure for the component:
...
import FreeNFTExtention from '@/abis/FreeNFT/FreeNFTExtention.json';
...
Add state variables and hooks to manage the functionality:
```typescript
const HomePage = () => {
...
Add the function to mint ERC20 tokens:
...
Add the function to fetch NFTs directly from the blockchain:
...
Add the JSX for the UI components:
...
1.  Connect your wallet using the sidebar connector
...
1.  User Operations vs. Direct RPC:
...
1.  Error Handling: Always implement proper error handling and loading states for a better user experience.

1.  Paymaster API Key: Make sure you have added a valid Paymaster API key in the nerowallet.config.ts file.

Congratulations! You've successfully enhanced the NERO Wallet template with NFT and token features!

<PageFeedback path="/en/tutorials/high-level/nero-wallet-template-quickstart" />