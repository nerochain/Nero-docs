# Types of NERO Chain Paymaster Operations

This tutorial explains the different types of payment operations supported by the NERO Chain Paymaster, their use cases, and how to implement them in your application.

## What You'll Learn
- The three payment types supported by the NERO Chain Paymaster
- When to use each payment type for different use cases
- How to implement free/sponsored transactions (Type 0)
- How to implement prepay ERC20 transactions (Type 1)
- How to implement postpay ERC20 transactions (Type 2)
...
- Completed the [Paymaster Integration](/en/tutorials/paymaster-integration) tutorial
- Understanding of React components and state management

## Understanding Paymaster Payment Types

The NERO Chain Paymaster supports three payment types for transaction gas fees:
...
1. **Start with Type 0** for new users to provide a seamless onboarding experience
2. **Transition to Type 1 or 2** for regular users based on your application's needs
3. **Always handle errors** gracefully, especially for token approval scenarios
4. **Provide clear feedback** to users about payment types and costs
5. **Monitor your API usage** to manage costs associated with Type 0 (sponsored) transactions

## Next Steps
Now that you understand the different payment types, you might want to learn how to check which tokens are supported by the NERO Chain Paymaster. Continue to the [Token Support Check](/en/tutorials/checking-sup-tokens) tutorial to learn how to query and display supported tokens for gas fee payments.